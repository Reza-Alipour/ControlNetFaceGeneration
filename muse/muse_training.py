################################################################################################################
#####                                                                                                      #####
#####    This file is an adaptation of the `train_amused.py` script from the huggingface/diffusers repo     #####
#####                                                                                                      #####
################################################################################################################
...
class AdapterDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        example = {}
        instance_image = self.ds['train'][index % self.num_instance_images][column_name].convert('RGB')
        caption = self.ds['train'][index % self.num_instance_images]['captions_all']
...
def parse_args():
    ...
    parser.add_argument("--push_to_hub_id", default=None, type=str)
...
def main(args):
    ...
    if args.seed is not None:
        set_seed(args.seed)
    if args.text_encoder_type == "xlm":
        text_encoder_class = RobertaSeriesModelWithTransformation
    elif args.text_encoder_type == "clip":
        text_encoder_class = CLIPTextModelWithProjection
    else:
        raise Exception("Unsupported text encoder")
    ...
    num_train_epochs = args.epochs if args.epochs is not None else math.ceil(
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch in train_dataloader:
            with torch.no_grad():
                ...
                caption_attentions = tokenized_captions.attention_mask.to(accelerator.device, non_blocking=True)
                outputs = text_encoder(
                    input_ids=caption_ids,
                    attention_mask=caption_attentions,
                    return_dict=True,
    ...
    if accelerator.is_main_process:
        ...
        if args.use_ema:
            ema.copy_to(model.parameters())
        model.save_pretrained(f'{args.output_dir}/last_checkpoint')
        if args.push_to_hub:
            upload_folder(
                repo_id=args.push_to_hub_id,
                folder_path=f'{args.output_dir}/last_checkpoint',
                token=args.write_token,
                revision=args.write_revision
            )
